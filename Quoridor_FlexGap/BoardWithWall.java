...
public class BoardWithWall extends Board{
    ...
    public String[] toRowString() {
        ...
        String gap = drawCellGap();
        String wall = drawCellWall();
        int k = 0;
        lines[k++] = line;
    ...
    public String drawCellWall() {
        String lengthStr = Integer.toString(getColumns() * getColumns());
        int len = lengthStr.length();
        String wall = "+";
        StringBuilder sbwall = new StringBuilder(wall);
        for (int gapIndex = 0; gapIndex < len; gapIndex++)
        {
            sbwall.append("#");
        }
        wall = sbwall.toString();
        return wall;
    }

    public String drawCellGap() {
        String lengthStr = Integer.toString(getColumns() * getColumns());
        int len = lengthStr.length();
        String gap = "+";
    ...
    public void clearCell(int index){
        ...
        int positionLength = Integer.toString(getColumns() * getColumns()).length();
        cell.setMarker(new Checker(index+1, positionLength));
    }
